...
//根据屏幕的实际宽高计算每一个grid和cell的宽高，
// 从而让页面显示符合移动端
function setWidthHeight(){

	//拿到2048的小方块棋盘的实际的宽高
	var width = window.innerWidth;
	var height = window.innerHeight-$('#scoreId').height();

	//计算cell_width cell_height
	//约定base_margin_left是cell_width的1/4
	//base_margin_left = cell_width/4;
	//width = 4*cell_width+5*(cell_width/4) = (21/4)*cell_width;
	 cell_width = (4/21)*width
	 cell_height = (4/21)*height;

	 base_margin_left = width/21;
	 base_margin_top = height/21;
	...
	for(var r=0;r<4;r++)
	{
		for(var c=0;c<4;c++)
		{
			//查找到g00 g01 c00 c01
			$('[id $= "'+r+c+'"]').css({
				top:base_margin_top*(r+1)+cell_height*r,
				left:base_margin_left*(c+1)+cell_width*c
			})
		}
	}
...
var game={
	...
	start:function(){//开始游戏方法
		...
		Panel.on('swipeLeft', function () {
			console.log('swipe to left triggered');
			this.moveLeft();
		}.bind(this))

		Panel.on('swipeRight', function () {
			console.log('swipe to right triggered');
			this.moveRight();
		}.bind(this))

		Panel.on('swipeUp', function () {
			console.log('swipe to up triggered');
			this.moveUp()
		}.bind(this))

		Panel.on('swipeDown', function () {
			console.log('swipe to Down triggered');
			this.moveDown();
		}.bind(this))
